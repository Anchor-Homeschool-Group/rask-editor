...
function addResponsiveToolbar(container: HTMLElement) {
  ...
  const controls = () =>
    Array.from(container.children).filter(
      (element): element is HTMLElement =>
        element instanceof HTMLElement && element !== moreButton,
    );

  controls().forEach((item) => {
    const hasPrimaryFormat =
      item.matches('.ql-bold, .ql-italic') ||
      item.querySelector('.ql-bold, .ql-italic') != null;
    item.classList.toggle('ql-toolbar-primary', hasPrimaryFormat);
  });

  let overflowItems = new Set<HTMLElement>();

  const setControlVisibility = (expanded: boolean) => {
    controls().forEach((item) => {
  ...
  const setExpanded = (expanded: boolean) => {
    ...
    moreButton.setAttribute(
      'aria-label',
      expanded
        ? 'Show fewer formatting options'
        : 'Show more formatting options',
    );
    moreButton.title = expanded
  ...
  const updateOverflow = () => {
    ...
    const firstRowTop =
      items.length > 0
        ? Math.min(...items.map((item) => item.offsetTop))
        : null;
    overflowItems = new Set(
      firstRowTop == null
        ? []
        : items.filter((item) => item.offsetTop > firstRowTop),
    );
